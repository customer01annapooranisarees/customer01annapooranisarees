...
const { info, error } = require('@customer01annapooranisarees/annapoorani/src/lib/log/logger');
...
function watchMR() {
  ...
  if (existsSync(resolve(CONSTANTS.ROOTPATH, 'packages'))) {
    watcher.add(
      resolve(
        CONSTANTS.ROOTPATH,
        'packages/annapoorani/src/modules/*/controllers/**'
      )
    );
    watcher.add(
      resolve(
        CONSTANTS.ROOTPATH,
        'packages/annapoorani/src/modules/*/apiControllers/**'
      )
    );
  }
