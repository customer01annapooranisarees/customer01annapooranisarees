...
const { getRoutes } = require('@customer01annapooranisarees/customer01annapooranisarees/src/lib/router/routes');
const {
  getComponentsByRoute
} = require('@customer01annapooranisarees/customer01annapooranisarees/src/lib/componee/getComponentByRoute');
const webpack = require('webpack');
const { info } = require('@customer01annapooranisarees/customer01annapooranisarees/src/lib/log/logger');
...
if (existsSync(path.resolve(CONSTANTS.ROOTPATH, './.customer01annapooranisarees/build'))) {
  rmdirSync(path.resolve(CONSTANTS.ROOTPATH, './.customer01annapooranisarees/build'), {
    recursive: true
  });
}
...
const {
  createVendorConfig
} = require('@customer01annapooranisarees/customer01annapooranisarees/src/lib/webpack/configProvider');
...
webpackVendorPromise.then(async () => {
  controllers.forEach((route) => {
    ...
    const buildFunc = async function () {
      ...
      content += '\r\n';
      await mkdir(
        path.resolve(CONSTANTS.ROOTPATH, './.customer01annapooranisarees/build', buildPath),
        { recursive: true }
      );
      await writeFile(
        path.resolve(
        ...
        plugins: [
          new webpack.DllReferencePlugin({
            manifest: path.resolve(
              CONSTANTS.ROOTPATH,
              './.customer01annapooranisarees/build/vendor-manifest.json'
            )
          })
        ]
      });
      ...
      spinner.succeed(
        green('Building completed!!!\n') +
          boxen(green('Please run "npm run start" to start your website'), {
            title: 'customer01annapooranisarees',
