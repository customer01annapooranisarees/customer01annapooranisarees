...
const { info, error } = require('@customer01annapooranisarees/customer01annapooranisarees/src/lib/log/logger');
...
function watchMR() {
  ...
  if (existsSync(resolve(CONSTANTS.ROOTPATH, 'packages'))) {
    watcher.add(
      resolve(
        CONSTANTS.ROOTPATH,
        'packages/customer01annapooranisarees/src/modules/*/controllers/**'
      )
    );
    watcher.add(
      resolve(
        CONSTANTS.ROOTPATH,
        'packages/customer01annapooranisarees/src/modules/*/apiControllers/**'
      )
    );
  }
