...
module.exports = exports = (request, response, next) => {
  ...
  } else if (existsSync(join(CONSTANTS.ROOTPATH, '.customer01annapooranisarees/build', path))) {
    staticMiddleware(join(CONSTANTS.ROOTPATH, '.customer01annapooranisarees/build'))(
      request,
      response,
      next
    );
