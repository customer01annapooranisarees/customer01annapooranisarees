...
module.exports = async (request, response, route) => {
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    if (route.isAdmin === true) {
      const bundles = readdirSync(
        path.resolve(CONSTANTS.ROOTPATH, './.customer01annapooranisarees/build/admin', route.id),
        { withFileTypes: true }
      )
      ...
      const bundles = readdirSync(
        path.resolve(
          CONSTANTS.ROOTPATH,
          './.customer01annapooranisarees/build/frontStore',
          route.id
        ),
        { withFileTypes: true }
      )
  ...
  if (
    existsSync(path.resolve(CONSTANTS.ROOTPATH, './.customer01annapooranisarees/build', scopePath))
  )
    await rmdir(
      path.resolve(CONSTANTS.ROOTPATH, './.customer01annapooranisarees/build', scopePath),
      { recursive: true }
    );
